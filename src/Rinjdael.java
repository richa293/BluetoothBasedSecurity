...
public class Rinjdael
{
	...
	private static int[] sbox = { 0x63, 0x7C, 0x77, 0x7B, 0xF2, 0x6B, 0x6F,
		...
		private static byte[] deletePadding(byte[] input) {
		 ...
		 void encryptFile(String filePath, String key) throws IOException
		{
								String k =key;
								...
								fis.read(dataBytes);
								fis.close();
								byte[] encryptedData = encrypt(dataBytes,k.getBytes());
								dataFile.delete();
								String encryptedDataFilePath = filePath+".enc";
								File encryptedFile= new File(encryptedDataFilePath);
								FileOutputStream fos = new FileOutputStream(encryptedFile);
								fos.write(encryptedData);
								encryptedFile.setReadable(false);
								encryptedFile.setWritable(false);
								
								fos.close();
				
					    
			}
		void encryptFile1(String filePath, String key) throws IOException
		{
								String k =key;
								String dataFilePath = filePath;
								File dataFile = new File(dataFilePath);
								...
								byte[] encryptedData = encrypt(dataBytes,k.getBytes());
								dataFile.delete();
								String encryptedDataFilePath = filePath;
								File encryptedFile= new File(encryptedDataFilePath);
								FileOutputStream fos = new FileOutputStream(encryptedFile);
								fos.write(encryptedData);
								encryptedFile.setReadable(false);
			...
			static void decryptFile(String filePath,String key) throws IOException
				{
					
					String k=key;
					String encryptedDataFile = filePath;
				    File dataFile = new File(encryptedDataFile);
				    FileInputStream fis = new FileInputStream(dataFile);
				    byte[] dataBytes = new byte[fis.available()];
				    fis.read(dataBytes);
				    fis.close();
				    byte[] decryptedData = decrypt(dataBytes,k.getBytes());
				    dataFile.delete();
				    int l=filePath.length();
				    filePath=filePath.substring(0,l-4);
				    String decryptedDataFilePath = filePath;
				    File decryptedFile=new File(decryptedDataFilePath);
				    FileOutputStream fos = new FileOutputStream(decryptedFile);
				    fos.write(decryptedData);
					decryptedFile.setReadable(true);
					decryptedFile.setWritable(true);
				    fos.close();
				}
			static void decryptFile1(String filePath,String key) throws IOException
			{
				String k=key;
				String encryptedDataFile = filePath;
			    File dataFile = new File(encryptedDataFile);
			...
			public static void main(String args[]) throws IOException
			{
				...
				String key="20547600E362";
				//encryptFile(File,key);				
				decryptFile1(File,key);
			}
